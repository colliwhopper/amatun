...
# allow selection of rds database on script run rather than just using "amatun" tags
...
if nc -z -w5 127.0.0.1 9990 >> /dev/null 2>&1;[ "$?" -eq 1 ]; then
  echo 'no existing SSM session detected on port 9990, continuing...'
  echo
elif  nc -z -w5 127.0.0.1 9990 >> /dev/null 2>&1;[ "$?" -eq 0 ]; then
...
do
	...
	if nc -z -w5 127.0.0.1 9990 >> /dev/null 2>&1;[ "$?" -eq 0 ]; then
	  echo
...
if nc -z -w5 127.0.0.1 3388 >> /dev/null 2>&1;[ "$?" -eq 0 ]; then
  'no existing SSH tunnel detected on port 3388, continuing...'
  break
elif nc -z -w5 127.0.0.1 3388 >> /dev/null 2>&1;[ "$?" -eq 0 ]; then
  ...
  sleep 5
  if nc -z -w5 127.0.0.1 3388 >> /dev/null 2>&1;[ "$?" -eq 1 ]; then
    echo
...
ssh -o "UserKnownHostsFile=/dev/null" -o "StrictHostKeyChecking=no" -o "LogLevel=ERROR" $SSHUSER@127.0.0.1 -p 9990 -N -L 3388:$DBID:3306 &
echo 'SUCCESS'
echo
